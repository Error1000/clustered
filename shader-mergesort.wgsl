...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if actual_id == 0 {
        out_data.input_a_size = total_input_size;
        out_data.input_b_size = total_input_size;
    }
    ...
    loop {
        if a_val < b_val {
            out_data.data[total_start_offset+out_indx] = a_val;
            out_indx += 1u;
            a_indx += 1u;
            if a_indx >= a_size { break; }
            a_val = in_data.data[a_start_offset+a_indx];
        }else {
            out_data.data[out_indx+total_start_offset] = b_val;
            out_indx += 1u;
            b_indx += 1u;
